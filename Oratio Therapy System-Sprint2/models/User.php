...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        $rules = [
            ...
            ['username','required'],  
            [['email', 'username'], 'unique', 'message' =>'Email già esistente'],
            [['email', 'username'], 'filter', 'filter' => 'trim'],
            [['email'], 'email'],
            [['username'], 'match', 'pattern' => '/^\w+$/u', 'except' => 'social', 'message' => Yii::t('user', '{attribute} può contenere solo lettere, numeri, and "_"')],
            ...
            [['newPasswordConfirm'], 'compare', 'compareAttribute' => 'newPassword', 'message' => Yii::t('user', 'Password non coincidente')],

            // account page
            [['nome', 'cognome', 'dataNascita', 'codFisc'], 'required'],
            [['currentPassword'], 'validateCurrentPassword', 'on' => ['account']],
            [['nome'],'safe'],          
            [['cognome'],'safe'],         
            [['dataNascita'],'safe'], 
            [['codFisc'], 'unique'],
            [['codFisc'], 'string', 'min' => 16, 'max'=>16] ,               
            [['cfUtAssociato'],'safe'],         
            [['cfLogAssociato'],'safe'],          
            [['cell'],'safe'],          
            ['codLicenza','required', 'when' => function($model){return $model->role_id==3;},],  
            [['codLicenza'], 'string', 'max' => 15],  
            ['codLicenza', 'unique'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            //Campi aggiunti
            'nome' => Yii::t('user', 'Nome'),           //Aggiunto per aggiungere campo nome
            'cognome' => Yii::t('user', 'Cognome'),           //Aggiunto per aggiungere campo cognome
            'dataNascita' => Yii::t('user', 'Data di nascita'),
            'codFisc' => Yii::t('user', 'Codice Fiscale'),
            'cfUtAssociato' => Yii::t('user', 'Codice Fiscale Utente Associato'),
            'cfLogAssociato' => Yii::t('user', 'Codice Fiscale Logopedista'),
            'cell' => Yii::t('user', 'Numero di cellulare'),
            'codLicenza' => Yii::t('user', 'Codice licenza logopedista'),
            //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            'auth_key' => Yii::t('user', 'Auth Key'),
            'access_token' => Yii::t('user', 'Access Token'),
            'logged_in_ip' => Yii::t('user', 'Logged In Ip'),
            'logged_in_at' => Yii::t('user', 'Ultimo Accesso'), //Logged In At
    ...
    public function setRegisterAttributes($roleId, $status = null)
    {
        ...
        //********************************************************************--------------------------------------------------
        //Commentato per bypassare conferma email
        if ($status) {
    ...
    public function OttieniutentiLogopedista($cflog)
    {
      
        $rows = (new \yii\db\Query())
            ->from('user')
            ->where(['role_id' => 3, 'cfLogAssociato' => $cflog  ])
            ->all();
        return $rows;

    }

    public function OttieniutentiLog($cflog)
    {
        $u = User::find()->where(['cfLogAssociato' => $cflog, 'role_id'=>2, 'status'=>1])->all();
        return $u;
    }
